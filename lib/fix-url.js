...
function fix(opts) {
  ...
  function processFontDecl(decl) {
    var src, hex, hexFile;
    decl.value = decl.value.replace(REG_URLS, function (all, $1, file, $3) {
      src = path.join(IMG_PATH, file);
      try {
        hex = cal(fs.readFileSync(src));
      } catch(e) {
        console.error('[Font No Exist]', e.message);
        hex = '';
      }
      ...
      return 'url(' + URL.resolve(opts.cdn || '../', 'img/' + _hexFile + ($3 || '')) + ')';
    });
  }

  function processDecl(decl) {
    var file = decl.value.match(REG_URL)[2]
      , url = cache[file]
      , bindary
      , src
      , hex
    ...
    if (url) {
      ...
      try {
        ...
        console.error('[Image No Exist]', e.message);
        hex = '';
      }
      hexFile = file.replace(REG_PREFIX, '.' + hex + '.$1');
      var _file = file.match(REG_FILE)[1]
  ...
  var plugin = postcss.plugin('postcss-fix-url', function (opts) {
    return function (css) {
      css.eachDecl(function (decl) {
        if (decl.value && ~decl.value.indexOf('url(')) {
          if (~decl.value.indexOf('../../img/fonts')) {
            processFontDecl(decl);
          } else if (~decl.value.indexOf('../../img')) {
            processDecl(decl);
          }
        }
      });
