...
var LazyBuilder = function() {};
LazyBuilder.prototype = {
    // default is null
    properties: null,
    // default is null
    path: null,
    ...
    _wrap: function (file, cb) {
        ...
        var handle = function () {
            ...
            console.log(name, shouldPath, md5.digest('hex').slice(0, 11));
...
if (spritesmith) {
    module.exports = function (param) {
        ...
        var sprite = function(param) {
            var lazyBuilder = new LazyBuilder();
            // file path
            ...
            LazyBuilderList.push(lazyBuilder);
        };
        var objType = Object.prototype.toString.call(param.path);

        if(objType.indexOf("String") !== -1) {
            sprite(param);
        } else if(objType.indexOf("Array") !== -1) {
            param.path.forEach(function(path) {
                sprite({
                    path: path
                });
            });
        ...
        var stream = map(function (file, fn) {
            var index = 0;
            LazyBuilderList.forEach(function(lazyBuilder) {
                index++;
                lazyBuilder.build(file, function(cbParam, file) {
                    if (index === LazyBuilderList.length) {
                        fn(null, file);
                    }
                });
            });
