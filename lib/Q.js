...
define(['jquery'], function ($) {
    ...
    extend(Q.prototype, {
            ...
            if (elList) {
                elList.forEach(function (elObj) {
                    ...
                    switch (ele.nodeType) {
                        // element
                        case 1:
                            ele.innerHTML = self.applyFilters(value, elObj.writeFilters);
                            break;
                    }
                });
            }
            ...
            _walk([el], function (type, node) {
                switch (type) {
                    case 'TEXT':
                        ...
                        if (
                            match &&
                            ~_slice.call(node.parentNode.classList, 0)
                                .indexOf('q-mark')
                        ) {
                            ...
                            elList.push({
                                ...
                                writeFilters: self._makeWriteFilters(params.slice(1))
                            });
        ...
        _makeWriteFilters: function (names) {
            if (!names.length) return undefined;
            var filters = this.$options.filters;
            return names.map(function (name) {
                return filters[name] && filters[name].write || _turnback;
            });
        },
