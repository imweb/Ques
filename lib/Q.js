...
define(['jquery'], function ($) {
    ...
    extend(Q.prototype, {
        ...
        _setValue: function (key, data, value) {
            // need to fix key
            if (~key.indexOf('.')) {
                var keys = key.split('.');
        ...
        data: function (key, value) {
            var self = this, obj;
            if (!value) {
                if (typeof key === 'string') return this._data[key];
                obj = key;
                for (key in obj) {
            ...
            this._setValue(key, this._data, value);
        ...
        _templateBind: function (el, options) {
            ...
            _walk([el], function (type, node, arg) {
                switch (type) {
                    case 'TEXT':
                        var descriptors = self._parse(arg);
                            ...
                            switch (obj.name) {
                                case 'q-text':
                                    self._parse(obj.value).forEach(function (descriptor) {
                                        ...
                                        function foo() {
                                            self.applyFilters(data[key], readFilters);
                                        }
                                        self.$watch(namespace + key, foo);
                                        foo();
                                    });
                                    break;
                                case 'q-value':
