var get = require('./utils/get')
  ...
  , config = require('./appConfig')(require('../config'))
...
function buildAsyncComponent(names, pipes, src, host, dist, urlMap, cb) {
  var resMap = {};

  each(names, function (name, i, done) {
    var md5Stream = md5()
      , newUrlMap = merge({}, urlMap);

    newUrlMap[url.resolve(host, _.getDep(name, 'render.js'))] = false;

    buildJS([url.resolve(host, _.getDep(name, 'render.js'))], newUrlMap, undefined, host, function (res) {
      multiPipe(
        gulp.src([path.join(src, 'index.html')])
          .pipe(map(function (file, cb) {
            // give a component path
            file.path = path.resolve(file.base, 'js/components/' + name.split('-').join('/') + '/' + name + '.js');
        ...
        .pipe(gulp.dest(dist))
        .on('end', function () {
          merge(resMap, md5Stream.fileMap);
          done();
        });
    });
  }, function () {
...
function buildJS(jses, urlMap, externals, host, cb) {
  ...
  each(jses, function (js, i, done) {
    var isLib = js.indexOf('http://') === -1
      // real lib is name module in the realPaths
      , isRealLib = !!(isLib && config.realPaths[js])
      , jsUrl = isLib ?
    ...
    get(jsUrl)
      .on('body', function (body) {
        ...
        // transform define([], factory) -> define(factory)
        body = body.replace(/define\((['"][^'"]+['"]\, *)?(\[\],? *)/, 'define($1');

        body.replace(/component\.loader\((['"])([\s\S]+?)\1\)/g, function (all, $1, $2) {
          // add all aysnc component
          self && self.asyncComponents &&
            self.asyncComponents.add($2);
        });

        // deal with library do not have name
        // we should transform define(factory) -> define(name, factory)
        if (isLib && !isRealLib) {
          body = body.replace(/define\((['"][^'"]+['"])?/, function (all, $1) {
            if ($1) {
              return all;
            } else {
              return "define('" + js + "', ";
        ...
        if (deps) {
          deps = deps[2]
            ...
            // ignore such like http://pub.idqqimg.com/qqmobile/qqapi.js?_bid=152
            .filter(function (dep) {
              return !(~dep.indexOf('http://') && !(~dep.indexOf(host)))
            });
        }

        if (!deps || !deps.length || isRealLib) {
          !isLib &&
          ...
          buildJS.call(self, deps, urlMap, externals, host, function (_res) {
            res.push.apply(res, _res);
            !isLib && urlMap[js] !== false &&
              (body = body.replace(/define\(/, "define('" + urlMap[js] + "', "));
            res.push(body);
            done();
          });
...
p._init = function (path, options) {
  ...
  this.asyncComponents = new TagSet();
      ...
      self._when('js', function (value, externals) {
        ...
        $('script[main]').replaceWith(
          '<script>{{script}}</script>'
            .replace(/\{\{script\}\}/, [
              'require.config({ baseUrl: "' + url.resolve(config.cdn, './js') + '", paths: ' + JSON.stringify(paths) + ', shim: ' + JSON.stringify(config.shim) + ' });',
...
p._packJS = function (js) {
  ...
  buildJS.call(self, jses, urlMap, undefined, self.host, function (res, externals) {
    res.push([
    ...
    var asyncComponents = self.asyncComponents.values();

    // has async Component
    if (asyncComponents.length) {
      buildAsyncComponent(asyncComponents, self.pipes['js'], self.src, self.host, self.dist, urlMap, function (componentMap) {
        res.unshift([
          "window._CM_ = " + JSON.stringify(componentMap) + ';'
        ]);
